...
if selected_name:
    ...
    if not name_data.empty:
        ...
        popularity_fig, popularity_ax = plt.subplots(figsize=(18, 9))  # Double the size for the popularity over time plot
        sns.lineplot(
            data=name_data,
            x='year',
            y='n',
        ...
        popularity_ax.set_title(f'Popularity of {selected_name} Over Time')
        popularity_ax.set_xlabel('Year')
        popularity_ax.set_ylabel('Count (n)')
        st.pyplot(popularity_fig)  # Pass the figure object to st.pyplot()
    else:
